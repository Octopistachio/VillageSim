...
var obj_type = string(object_get_name(object_get_parent(obj)));
...
for(var i = 0; i < ds_list_size(inv); i++) { //For each inventory slot
	...
	var current_obj_type = current_obj_prop[2] //Get the item's type
...
if(!foundMatch) { //If the object is NOT in the player's inventory
	...
	obj_prop[2] = obj_type; //Set the item's third property to its item type
