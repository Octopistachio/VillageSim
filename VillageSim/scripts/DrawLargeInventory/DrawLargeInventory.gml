...
if(!ds_list_empty(global.inventory_ds_list)) { //If the list isn't empty
	...
	for(var i = 0; i < slots_total; i++) { //For each row in the inventory			
		
		if(i % slots_wide == 0) { //If the counter has reached the number of slots wide
			currentRow++; //Decrease the row count
			i = 0; //And reset the column count
		}
			
		if(currentRow > slots_high) break; //If the number of rows is < 0, stop drawing
		...
		if(sprite_exists(current_obj_sprite)) { //If the sprite exists
			var sprite_x = invBorder_x + (inv_width/slots_wide) * (i+1) - (inv_width/slots_wide)/2;
			var sprite_y = win_height - invBorder_y + 32 - (64 * currentRow);
